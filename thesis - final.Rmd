...
For this study, 14.396 patients were eligible. After excluding patients with missing serum Na at listing, 5223 patients were included. See Figure \@ref(fig:meldna-fig1). The baseline characteristics of included patients at first active listing are shown in Table \@ref(tab:meldna-tab1). The median lab MELD score was 16 (IQR 11-21) and the median sodium concentration was 137 (IQR 134-140) mmol/L. Hyponatremia of \<135, \<130, and \<125 mmol/L was found in respectively 28.5%, 8.8%, and 2.6% of the patients. Patients with alcohol-induced cirrhosis (ALD) had the lowest median Na levels, see Figure \@ref(fig:meldna-fig2). 
...
```{r meldna-fig6, message=F,warning=F, echo=F, fig.cap='Calibration plot of the MELD-Na model showing the predicted and observed risks of death per decile (10%) of the patient population. The diagonal line represents perfect calibration.', fig.pos="!h"}
options(warn=-1)
library(lemon)
...
```{r meldna-fig7, echo=F,fig.ncol=1,fig.subcap=c('Gained MELD-Na points for each combination of MELD and serum sodium level at listing', 'Difference in predicted death probability between MELD and MELD-Na'), fig.cap='Point and risk differences between MELD and MELD-Na.'}
library(viridis)
library(RColorBrewer)
...
# knitr::include_graphics(c("figures/jm/figure 2a.png",
#                           "figures/jm/figure 2b.png"), auto_pdf=T)
options(warn=-1)
load("figures/jm/sprobs_t.Rdata")
load("figures/jm/meldnasprobs_t.Rdata")
load("figures/jm/subset.Rdata")
LM=0 ; t=90
scores <- Score(list("MELDNa-JM" = 1-sprobs_t,"MELD-Na"= 1-meldnasprobs_t), # vector of probabilities
  formula = Hist(days,binstatus) ~ 1, data = subset, # data in which the JM and cox test
  null.model = T, contrasts = T, conf.int= T, times = LM + t, # LM time + interval of prediction
  plots = c("roc","calibration"))
plotROC(scores,brier.in.legend = T,cens.method="local")
plotCalibration(scores, method="quantile",cens.method="local")
options(warn=0)
``` 
...
options(knitr.kable.NA = '')
kbl(tab1,booktabs=T, caption = "Characteristics of prioritized recipients.",
    linesep= "")%>%
  kable_styling(latex_options = c("scale_down","striped")) %>%
...
load("figures/jm/longjmprio.Rdata")
load("figures/jm/longmeldnaprio.Rdata")
A <- longjmprio %>% 
  mutate(id=factor(id)) %>% 
  group_by(id) %>% 
  ggplot(aes(x=daycount,y=meldna, group=id))+
  geom_line(color="black", alpha=0.4) + 
  geom_segment(aes(x = 0, y = 24, xend = 90, yend = 33),color="red", lwd=2)+
  #stat_smooth(aes(group = 1), color="red", se=F, method="lm", lwd=2) +
  scale_x_continuous("Days on the waiting list", breaks=c(0,30,60,90), 
                     labels=c(0,30,60,90), limits=c(0,90))+
  scale_y_continuous("MELD-Na score", limits=c(6,40))+
  ggtitle("MELDNA-JM")+
  theme_pubr() 

B <- longmeldnaprio %>% 
  mutate(id=factor(id)) %>% group_by(id) %>% 
  ggplot(aes(x=daycount,y=meldna, group=id))+
  geom_line(color="black", alpha=0.4) + 
  stat_smooth(aes(group = 1), color="red", se=F, method="lm", lwd=2) +
  scale_x_continuous("Days on the waiting list", breaks=c(0,30,60,90), 
